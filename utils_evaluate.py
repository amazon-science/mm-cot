...
def get_acc_with_contion(res_pd, key, values):
    ...
    correct_pd = total_pd[total_pd['true_false'] is True]
...
def get_scores(result_data, rationale_data, results_reference, data_file):
    ...
    # accuracy scores
    acc_average = len(res_pd[res_pd['true_false'] is True]) / num * 100
    ...
    return {
        "answer": {
            'acc_natural': get_acc_with_contion(
                res_pd, 'subject', 'natural science'
            ),
            'acc_social': get_acc_with_contion(
                res_pd, 'subject', 'social science'
            ),
            'acc_language': get_acc_with_contion(
                res_pd, 'subject', 'language science'
            ),
